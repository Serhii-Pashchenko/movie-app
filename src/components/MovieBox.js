...
const MovieBox = ({id, title, poster_path, genre_ids, vote_average, release_date, overview, genres}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`https://api.themoviedb.org/3/movie/${id}/casts?api_key=e4016d7a28f89745bc184dfb96de3b98`);
          ...
          <Modal show={show} onHide={handleClose}>
            ...
            <Modal.Body>
              ...
              <div className="actors">
                {error ? <p>Помилка сервера: {error.message}</p> :
                  actors ? (actors.map(actor => (
                    <div className="actors item" key={actor.id}>
                      {actor.profile_path ? (<img src={API_IMG + actor.profile_path} alt={actor.name}/>) : (actor.gender === 1 ? <img src={NoPhotoWoman} alt={actor.name}/> : <img src={NoPhotoMan} alt={actor.name}/>)}
                      <div>
